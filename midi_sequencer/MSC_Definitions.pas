...
begin
   Result := Format ('%d', [Data_Byte_1]);
   case Event_Type of
      mc_MIDI_Control_Change: Result := Format ('Controller %3d', [Data_Byte_1]);
      mc_MIDI_Program_Change: Result := Format ('Patch %3d', [Data_Byte_1]);
   end; // case
end; // get_text_data_byte1 //

function TMIDI_Event.get_text_data_byte2: AnsiString;
begin
   Result := Format ('%d', [Data_Byte_2]);
...
begin
   ...
   case Data_Byte_1 of
      mc_Meta_Sequence_Number:   if Length (s) = 2
         then t := Format ('Sequence number: %d', [convert_to_word (Ord (AnsiChar (s [1])), Ord (AnsiChar (s [2])))])
         else t := Format ('Sequence number: %s', [s]);
      mc_Meta_Text_Event:        t := Format ('Text: %s', [s]);
      mc_Meta_Copyright_Notice:  t := Format ('Copyright: %s', [s]);
      mc_Meta_Track_Name:        t := Format ('Track name: %s', [s]);
      mc_Meta_Instrument_Name:   t := Format ('Instrument name: %s', [s]);
      mc_Meta_Lyric:             t := Format ('Lyric: %s', [s]);
      mc_Meta_Marker:            t := Format ('Text marker: %s', [s]);
      mc_Meta_Cue_Point:         t := Format ('Cue point: %s', [s]);
      mc_Meta_Channel_Prefix:    t := Format ('Channel prefix: %s', [s]);
      mc_Meta_End_Of_Track:      t := Format ('EOT', []);
      mc_Meta_Set_Tempo:         t := Format ('Set tempo: %d', [convert_to_cardinal (0, Ord (s [1]), Ord (s [2]), Ord (s [3]))]);
      mc_Meta_SMPTE_Offset:      t := Format ('SMPTE offset: %2.2x %2.2x %2.2x %2.2x %2.2x', [Ord (s [1]), Ord (s [2]), Ord (s [3]), Ord (s [4]), Ord (s [5])]);
      mc_Meta_Time_Signature:    t := Format ('Time signature: %2.2d %2.2d %2.2d %2.2d', [Ord (s [1]), Ord (s [2]), Ord (s [3]), Ord (s [4])]);
      mc_Meta_Key_Signature:     t := Format ('Key signature: %2.2x %2.2x', [Ord (S [1]), Ord (s [2])]);
      mc_Meta_Sequencer_Specific:t := Format ('Sysex 7F: %s', [s]);
      else                       t := Format ('Meta (%d) %s', [Data_Byte_1, s]);
   end; // case
   Result := t;
end; // get_text_translated //
